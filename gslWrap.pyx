...
cdef double betaincinv(double a, double b, double p, double acc = 1.e-12):
    ...
    if a+b>=5:           
        if (b-a)<=sqrt(a):
            ...
            if p**(1./a)<0.0025:
                ...
                eps3 = ((3704.*s2*s2*s2*s2*s2-9620.*s2*s2*s2*s2+6686.*s2*s2*s2-769.*s2*s2-1259.*s2+449.)/(102060.*s2*s2*s*c2*c2*c) -
                        ...
                        (729754.*s2*s2*s2*s2*s2*s2*s2-78755.*s2-2554139.*s2*s2*s2*s2*s2*s2+146879.*s2*s2*s2-1602610.*s2*s2*s2*s2+3195183.*s2*s2*s2*s2*s2+105222.*s2*s2+29233.)/(36741600.*s2*s2*s2*s*c2*c2*c2*c)*eta0*eta0)
        ...
        if p>gsl_sf_beta_inc(b,a,(1.-b)/(2.-b-a)):#p>gsl_sf_beta_inc(a,b,(1.-a)/(2.-a-b)):
            temp = (b*q*gsl_sf_beta(b,a))**(1./b)
...
#     if q<=100*machineEpsilon:
#         xr = 1.
#         xl = 0.
#         x = 0.5
#         for jj in range(nIterBisect):
